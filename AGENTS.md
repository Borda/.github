...
01. **Context is King**: Always prioritize the local project's `README` and `CONTRIBUTING` files.
02. **Suspicion is a Virtue**: Verify inputs, verify outputs, and verify environment assumptions.
03. **Clarity > Cleverness**: Write code that is easy to debug, even if it is slightly more verbose.
04. **Logging**: All complex logic must emit logs; "silent failure" is strictly forbidden.
05. **Documentation**: Document all functions, classes, and modules with clear, concise comments.
06. **Test-Driven Development**: Write tests before implementing or fixing bugs.
07. **Continuous Integration**: Ensure all changes pass CI checks before merging into `main`.
08. **Code Reviews**: All PRs must have at least one review before merging.
09. **No Surprises**: Avoid making changes without prior discussion or approval.
